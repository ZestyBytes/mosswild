(function(root){
'use strict';
const members=['Pip','Emberkin','Dewdrop','Lunamoth','Bramble','Talus','Fennel','Pearl','Nova'],items=['berries','wood','fish','crystals'];
...
function clean(raw,now=Date.now()){
 ...
 for(const id of ['orchard','highlands','marsh','coast','observatory']){const o=f.orders?.[id];if(o&&Number.isInteger(o.tier)&&o.tier>=0&&o.tier<=10000){const start={};for(const k of ['berries','wood','fish','crystals','herbs','ore','shells','harvested','crafted','visits'])start[k]=Math.floor(number(o.start?.[k]));s.frontier.orders[id]={tier:o.tier,kind:['gather','farm','discover','craft'].includes(o.kind)?o.kind:'gather',start};}}
 ...
 const vq=raw.observatory||{};s.observatory={stage:Math.floor(number(vq.stage,0,6)),crystalStart:Math.floor(number(vq.crystalStart)),markers:[...new Set((Array.isArray(vq.markers)?vq.markers:[]).filter(n=>Number.isInteger(n)&&n>=0&&n<3))]};
 return s;
}
