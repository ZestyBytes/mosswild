(function(root){
'use strict';
const members=['Pip','Emberkin','Dewdrop','Lunamoth'],items=['berries','wood','fish','crystals'];
...
function clean(raw,now=Date.now()){
 ...
 s.position=raw.position&&['valley','meadow','grove','home','cottage','riverbank','cavern','orchard','highlands','marsh','coast','observatory'].includes(raw.position.area)?{area:raw.position.area,x:number(raw.position.x,208,2000),y:number(raw.position.y,368,2000)}:null;
